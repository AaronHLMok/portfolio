...
/*
 *New
 *Pop head will remove head of list given and return that head
 */
taskval_t *pop_head(taskval_t **list) {
	taskval_t *temp = *list;
...
/*
 *Readd will take current head of ready_q and add it to the end if task is yet to be completed
 */
taskval_t *readd(taskval_t* rdy) {
	taskval_t* temp = rdy;
	rdy = remove_front(rdy);
	rdy = add_end(rdy, temp);
	return rdy;
}